...
if(require(dplyr)) {
  data <- scof2012 %>%
    dplyr::mutate(data.type = rep(1, dim(scof2012)[1]), .before = psi)
}
...
for (ii in 1:length(species_list)){
  ...
  Linear_fits = fit_linear(data_by_sp, plot=T, xlab="Water Potential (MPa)", ylab="Leaf Hydraulic Conductance (mmol m^-2 s-1)")
...
for (ii in 1:length(species_list)){
  ...
  Logistic_fits = fit_nonlinear(data_by_sp, model_type = "log", plot=T,xlab="Water Potential (MPa)", ylab="Leaf Hydraulic Conductance (mmol m^-2 s-1)")#parsL, par_loL, par_highL,
...
for (ii in 1:length(species_list)){
  ...
  Exponential_fits = fit_nonlinear(data_by_sp,model_type = "exp", plot=F, xlab="Water Potential (MPa)", ylab="Leaf Hydraulic Conductance (mmol m^-2 s-1)")
