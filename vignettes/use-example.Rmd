...
cebe <- hydrafit::cebe_efm %>%
  mutate(data.type = rep(1, dim(cebe_efm)[1]), .before=psi) 
...
species_list <- unique(cebe$species)
```
...
for (ii in 1:length(species_list)){
  (subset(cebe, cebe$to_split == species_list[ii], select =c(1:5)) -> data_by_sp)
