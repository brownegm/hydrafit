test_that("bootstrap check", {
  ...
  for (ii in seq_along(unique(data$species))) {
    subset(data,
           data$species == unique(data$species)[ii],
           select = c(1:ncol(data))) -> data_by_sp

    linear_fits[[ii]] = hydrafit::fit_vuln_curve(
      formula,
      input_df = data_by_sp,
      model_type = "Linear",
      max_cond_at = 0.1,
  ...
  # run resample
  best_model <- fx_select(linear_fits,
                          logistic_fits,
                          sigmoidal_fits,
                          exp1_fits,
                          exp2_fits)
  ...
  testthat::expect_true(all(round(resample_test$psi_px, 5) == 0.1))
  ...
  #check the change
  testthat::expect_equal(updated_bootstrap_list[[3]]$margin_error,
                         exp2_fits[[3]]$margin_error)
...
test_that("Psi_max is provided to the resample function", {
  expect_error(resamplePX(
    c("this", "is", "a", "fake", "fit"),
    seed = 123,
    psi_max = NULL
  ))
